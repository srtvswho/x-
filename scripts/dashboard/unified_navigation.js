...
async function focusBacktest(){
  ...
  const d=await readData('/data/post-incremental-summary.json.gz'),g=d.modes.frozen_quarter.groups,r=d.modes.rolling_at_post.groups;
  const row=(title,p,benchmark)=>`<tr><td>${esc(title)}</td><td>${p.n}</td><td>${focusPP(p.excess_median)} / ${benchmark}</td></tr>`;
  app.innerHTML=`<div class="page"><div class="page-head"><h1>三个口径的历史回放</h1><p>60 个交易日 · 截至 2026-09-09 的成熟结果</p></div><div class="table-wrap"><table class="table"><thead><tr><th>样本组</th><th>成熟信号数</th><th>中位超额</th></tr></thead><tbody>${row('全部首次看多',g.all_first_us['60'],'SPY')}${row('领域内新观点',g.field_discovery['60'],'SOXX')}${row('季度历史达标',g.field_positive['60'],'SOXX')}${row('发帖前历史达标',r.field_positive['60'],'SOXX')}</tbody></table></div><p class="assessment-note">三个领域口径在这份历史样本里表现为正，但互相重叠，样本较少且领域来自事后复评。这是诊断回放，不是下一条观点的上涨概率。</p><p>标签从后续新观点中继续生成；重复喊单不继承首次观点的回测成绩。</p><div class="focus-actions"><a class="btn primary" href="/?scope=history">查看已标注历史观点</a><a class="btn" href="/reports/post-incremental-value.html">完整方法与失败样本</a><a class="btn" href="/reports/focus-backtest.html">原 v1 零信号报告</a></div></div>`;
}
const researchLabelNames = {field_discovery:'领域内新观点',quarter_supported:'季度历史达标',rolling_supported:'发帖前历史达标'};
function researchPostBadges(p){
  return (p.research_labels||[]).map(a=>`<div class="research-post-labels"><a href="/?scope=history&post=${encodeURIComponent(p.id)}#research-${encodeURIComponent(a.id)}"><strong>${esc(a.ticker)}</strong> · ${a.tags.map(t=>esc(researchLabelNames[t]||t)).join(' · ')}</a><span class="focus-note">首次观点标签 · ${esc(a.semantic_origin)}</span></div>`).join('');
}
function researchProfile(p,title){
  return `<details><summary>${esc(title)} · ${esc(p.status)} · ${p.n} 标的 / ${p.posts} 帖子</summary><p>评分截止 ${esc(fmtDate(p.cutoff))}；60 个交易日 · SOXX 中位超额 ${focusPP(p.median_excess)}。只用截止日前已成熟结果，排除候选自身。</p>${p.samples.length?`<div class="table-wrap"><table class="table"><thead><tr><th>历史标的</th><th>原帖日</th><th>观察结束</th><th>行业超额</th></tr></thead><tbody>${p.samples.map(e=>`<tr><td>${esc(e.ticker)}</td><td><a href="/posts/${encodeURIComponent(e.post_id)}">${esc(e.published_at.slice(0,10))}</a></td><td>${esc(e.exit_date)}</td><td>${focusPP(e.excess)}</td></tr>`).join('')}</tbody></table></div>`:'<p>当时尚无完整成熟的跨标的证据。</p>'}</details>`;
}
function researchCard(a,read){
  return `<article class="card research-card" id="research-${esc(a.id)}"><div class="meta"><span>${esc(fmtDate(a.published_at))}</span><span>${a.withdrawn?'后续观点已转向':a.is_live?'近期新观点':a.backfill?'历史补录':'历史首次观点'}</span>${!read.has(a.id)&&a.is_live?'<span class="chip">未读</span>':''}</div><h2>${esc(a.author)} · ${esc(a.ticker)}</h2><div class="research-tags">${a.tags.map(t=>`<span class="research-tag ${esc(t)}">${esc(researchLabelNames[t])}</span>`).join('')||'<span class="chip">待核实</span>'}</div><p class="focus-note">${esc(a.field_label)} · ${esc(a.semantic_origin)} · 已存历史内首次看多</p>${a.issues.length?`<p class="assessment-note">${a.issues.map(esc).join('；')}</p>`:''}${a.score_freshness_note?`<p class="assessment-note">${esc(a.score_freshness_note)}</p>`:''}${researchProfile(a.quarter_profile,'季度初历史成绩')}${researchProfile(a.rolling_profile,'发帖前历史成绩')}<details><summary>查看这次原文</summary><blockquote>${esc(a.raw_text)}</blockquote></details><div class="focus-actions"><a class="btn primary" href="/posts/${encodeURIComponent(a.post_id)}">原帖与 DeepSeek 解读</a><a class="btn" href="/posts/?ticker=${encodeURIComponent(a.ticker)}">该标的全部推文</a>${a.is_live&&!read.has(a.id)?`<button class="btn" data-focus-read="${esc(a.id)}">标为已读</button>`:''}</div></article>`;
}
async function focusHome(){
  nav('focus');document.title='重点关注 · SignalBoard';
  const data=await readData('/data/focus-signals.json.gz'),d=data.research_labels,read=focusRead(),q=new URLSearchParams(location.search);
  if(!d)throw new Error('研究标签正在更新，请稍后刷新。');
  const scope=q.get('scope')==='history'?'history':'recent',filter=q.get('label')||q.get('level')||'all',post=q.get('post');
  const candidates=d.alerts.filter(a=>scope==='history'||a.recent&&!a.backfill&&!a.withdrawn),counts=scope==='history'?d.history_counts:d.counts;
  const rows=candidates.filter(a=>(filter==='review'?!a.tags.length:filter==='all'?a.tags.length:a.tags.includes(filter))&&(!post||a.post_id===post));
  app.innerHTML=`<div class="page"><div class="page-head"><h1>可信作者的新标的</h1><p>后续新观点按三个口径标注，点开可核对当时的历史证据。</p></div><p class="assessment-note">三个标签可重叠，也都要求作者首次明确看多该标的。历史正超额用于研究参考，不代表买入胜率。<a href="/focus/backtest/">查看修正后的回测</a></p><div class="focus-actions"><a class="btn ${scope==='recent'?'primary':''}" href="/">近 ${d.recent_days} 天</a><a class="btn ${scope==='history'?'primary':''}" href="/?scope=history">历史符合样本</a><a class="btn" href="/posts/">查看全部推文</a></div><div class="focus-counts">${Object.entries(researchLabelNames).map(([key,name])=>`<a href="/?scope=${scope}&label=${key}" ${filter===key?'aria-current="page"':''}><strong>${counts[key]||0}</strong>${esc(name)}</a>`).join('')}<a href="/?scope=${scope}">全部符合</a><a href="/?scope=${scope}&label=review">待核实 ${candidates.filter(a=>!a.tags.length).length}</a></div><p class="focus-note">${rows.length} 条${scope==='history'?'历史（含早期与未成熟样本）':'近期'}首次观点 · 标签数量有重叠，不能相加 · 价格证据截至 ${esc(d.evidence_prices_as_of||'缺失')}</p>${post?'<p><a href="/?scope=history">清除原帖筛选</a></p>':''}${d.evidence_stale?'<p class="assessment-note">价格证据较旧，达标计算可能遗漏最近成熟结果，请先复核。</p>':''}<div class="list">${rows.map(a=>researchCard(a,read)).join('')||`<div class="empty">${scope==='recent'?'近 7 天暂无符合该口径的新标的。可切换“历史符合样本”查看已标注案例；新帖子随每日更新继续判断。':'这个口径暂无符合记录。'}</div>`}</div><details class="panel" style="margin-top:24px"><summary>作者能力领域与提醒门槛</summary><p>${esc(d.policy)}</p><p>领域内新观点不要求已有足够的历史成绩。两个历史达标标签分别固定在季度初和发帖前，不会用后来的涨跌回填当时标签。同一标的的重复看多留在推文中，不会反复生成新标的提醒。</p><p>历史样本经过人工复核；新增记录使用已存解读与明确证券表述核验，模糊条件和归属进入待核实。</p></details></div>`;
