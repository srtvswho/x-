"""P4-17 写修订报告 - MU 6-26 误抽修正 + QCOM 5-22 确认正确"""
import os, json
...
with open("/workspace/logs/p4p16_fixed_verified.json") as f:
    d = json.load(f)
...
md = []
md.append(f"""# P4-17 报告: MU 6-26 误抽修正 + Jukan 记分牌 v3
...
# 列出 3 条 short 详情
shorts = [p for p in his_v if p["verification"].get("direction") == "short"]
for p in shorts:
    v = p["verification"]
    md.append(f"### {p['source_date'][:10]} {v['ticker']} short {v['horizon_days']}d\n\n")
    md.append(f"**raw**: {v['SOXX']['raw_ret']:+.1f}% | **excess vs SOXX**: {v['SOXX']['excess_ret']:+.1f}% | **hit vs SOXX**: {'✅' if v['SOXX']['hit'] else '❌'}\n\n")
    md.append(f"**thesis**: {p.get('thesis','')}\n\n")
    md.append(f"**性质**: 局部风险提示 (specific product/contract concern), 不是全面看空建仓\n\n\n")
...
# 写
out_path = "/workspace/outputs/phase4_p17_jukan_scoreboard_v3.md"
with open(out_path, "w") as f:
    f.writelines(md)
print(f"✅ saved {out_path}")
print()
print("--- P4-17 关键 ---")
print("MU 6-26 误抽, drop")
print("raw 选股命中率 28/29 = 96.6% (从 93.3% 提升)")
print("他 vs 搬运 raw 优势 +37.4pp (从 +30pp 提升)")
print("他做 short 只剩 3 条 (都是局部风险, 不是全面做空)")
print("\"short 是弱项\" 结论不成立 (样本太小)")