"""P4-2d 把 since/until 嵌入 searchTerms 字符串,actor 才会传给 Twitter search query"""
...
def start_search(query, since, until, max_items=500):
    # 关键: 把 since/until 直接嵌入 searchTerms (Twitter 高级搜索语法)
    full_q = f"({query}) since:{since} until:{until}"
    inp = {
        "searchTerms": [full_q],  # 完整字符串
        "maxItems": max_items,
        "sort": "Latest",
        "tweetLanguage": "en",
    ...
    print(f"  [start] {full_q[:50]} → run={run.id[:8]}")
    ...
    while time.time() - t0 < max_wait:
        try:
            ...
            if cnt != last:
                print(f"    items: {cnt}")
                last = cnt
            if cnt >= 100 and time.time() - t0 > 30:
                time.sleep(8)
...
for ticker, (since, until) in WINDOWS.items():
    ...
    for q in QUERIES[ticker]:
        try:
            run_id, ds_id = start_search(q, since, until, max_items=500)
            items = poll_dataset(ds_id, max_wait=80)
            # Check time distribution
            dates = [(it.get("createdAt") or "")[:10] for it in items if it.get("createdAt")]
            if dates:
                in_window = sum(1 for d in dates if since <= d <= until)
                print(f"  → {len(items)} total, {in_window} in [{since},{until}]")
                print(f"    date_range: {min(dates)} ~ {max(dates)}")
            results.append({
                "ticker": ticker, "query": q, "run_id": run_id,
                "ds_id": ds_id, "items": items
            })
        except Exception as e:
            print(f"  ✗ {e}")
...
with open("/workspace/logs/p4p2d_results.json", "w") as f:
    json.dump({
        "windows": WINDOWS,
        "queries": QUERIES,
        "authors_earliest": list(authors_earliest.values()),
        "all_tweets_in_window": all_tweets,
    }, f, indent=2)
print(f"\n✅ saved /workspace/logs/p4p2d_results.json")