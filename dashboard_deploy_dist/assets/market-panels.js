...
function renderAuthorBrief(key,b){
  ...
  const paragraphs=b.paragraphs.map(p=>`<p>${escHtml(p.text)} ${p.sources.map(r=>briefLink(r)).join(' ')}${p.coverage?`<span class="brief-coverage">${escHtml(p.coverage)}</span>`:''}</p>`).join('');
  ...
  const securities=groups.map(([dir,label])=>{
    ...
    const tag=e=>`<a class="security-view" href="/posts/${encodeURIComponent(e.post_id)}"><b>${escHtml(e.ticker)}</b>${dir==='conditional'?` · ${directionLabel(e)}`:''}<span>${escHtml(e.date.slice(0,10))}</span></a>`;
    const tags=rows.slice(0,6).map(tag).join('')+(rows.length>6?`<details class="brief-more"><summary>展开其余 ${rows.length-6} 个</summary>${rows.slice(6).map(tag).join('')}</details>`:'');
    return `<div class="security-group ${dir}"><strong>${label}${rows.length?' · '+rows.length:''}</strong><div>${rows.length?tags:'<span class="brief-muted">暂无明确记录</span>'}</div></div>`;
  }).join('');
  const changes=b.changes.length?`<details class="brief-changes"><summary>${b.changes.length} 处标的方向记录不同 · 查看前后依据</summary><p class="brief-muted">短期谨慎与长期看好可以并存。这里只对照记录，不自动判断已经改口。</p>${b.changes.map(e=>`<div class="brief-change"><b>${escHtml(e.ticker)}</b><p>${directionLabel(e.before)} · ${escHtml(e.before.horizon)} ${briefLink(e.before)}<br>${escHtml(e.before.text)}</p><p>${directionLabel(e.after)} · ${escHtml(e.after.horizon)} ${briefLink(e.after)}<br>${escHtml(e.after.text)}</p></div>`).join('')}</details>`:'<p class="brief-muted">所选范围内，未发现同一标的明确多空记录的变化；不代表所有观点都保持不变。</p>';
  const latest=b.latest?`<div class="brief-latest"><strong>最近一次说到</strong><p>${escHtml(b.latest.text)} ${briefLink(b.latest)}</p></div>`:'';
  ...
  return `<article class="stance author-brief" data-author="${escHtml(key)}"><div class="sh"><div class="av" style="background:${c}">${initials(k.name)}</div><div class="nm">${escHtml(k.name)}</div><a class="brief-posts" href="/posts/?author=${encodeURIComponent(key)}">全部推文 ↗</a></div>
    <div class="brief-body"><div class="brief-meta">${winLabel(win)} · 已存 ${b.post_count} 篇 · ${b.interpreted_count} 篇有观点解读${b.last_post_at?' · 最新 '+escHtml(b.last_post_at.slice(0,10)):''}</div>
    <div class="brief-prose">${paragraphs}</div>${latest}
    <h3>观点的变化与延续</h3>${changes}${timeline}
    <h3>具体标的怎么看</h3><p class="brief-muted">以下为所选范围内最后一次明确表态，日期是发帖日，不代表当前持仓或买入建议。</p>${securities}${mention}
    </div></article>`;
}
