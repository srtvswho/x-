...
/* ---- 02 近期态度 ---- */
let win=1,stanceAuthor='all';
function briefLink(source,label){
  return `<a class="brief-source" href="/posts/${encodeURIComponent(source.post_id)}">${escHtml(label||source.date.slice(0,10))} 原帖 ↗</a>`;
}
function directionLabel(e){return (e.conditional?'条件性':'')+(e.direction==='long'?'看多':'看空');}
function renderAuthorBrief(key,b){
  const k=KOLS[key],c=COLORS[key];
  const evidence=p=>`<div class="brief-evidence">${p.coverage?`<span class="brief-coverage">${escHtml(p.coverage)}</span>`:''}${p.sources.map(r=>briefLink(r)).join(' ')}</div>`;
  const topics=(b.topics||[]);
  const mainParagraphs=topics.length?b.paragraphs.slice(1,1+topics.length):b.paragraphs;
  const paragraphs=mainParagraphs.map((p,i)=>{
    const heading=topics[i]||'本期观点';
    const prefix='关于'+heading+'，';
    const body=p.text.startsWith(prefix)?p.text.slice(prefix.length):p.text;
    return `<section class="brief-topic"><h4>${escHtml(heading)}</h4><p>${escHtml(body)}</p>${evidence(p)}</section>`;
  }).join('');
  const additional=topics.length?b.paragraphs.slice(1+topics.length):[];
  const extra=additional.length?`<details class="brief-extra"><summary>补充观点 · ${additional.length} 条</summary>${additional.map(p=>`<p>${escHtml(p.text)}</p>${evidence(p)}`).join('')}</details>`:'';
  const groups=[['long','看多'],['short','看空'],['conditional','条件性观点']];
  const securities=groups.map(([dir,label])=>{
    const rows=b.securities.filter(e=>dir==='conditional'?e.conditional:!e.conditional&&e.direction===dir);
    const tag=e=>`<a class="security-view" href="/posts/${encodeURIComponent(e.post_id)}"><b>${escHtml(e.ticker)}</b>${dir==='conditional'?` · ${directionLabel(e)}`:''}<span>${escHtml(e.date.slice(5,10))}</span></a>`;
    const tags=rows.slice(0,4).map(tag).join('')+(rows.length>4?`<details class="brief-more"><summary>展开其余 ${rows.length-4} 个</summary>${rows.slice(4).map(tag).join('')}</details>`:'');
    return `<div class="security-group ${dir}"><strong>${label}<span>${rows.length} 个</span></strong><div>${rows.length?tags:'<span class="brief-muted">暂无明确记录</span>'}</div></div>`;
  }).join('');
  const changes=b.changes.length?`<p class="brief-change-status">${b.changes.length} 处标的方向记录不同</p><p class="brief-muted">期限或条件可能不同，需对照前后依据。</p><details class="brief-changes"><summary>查看前后表态</summary>${b.changes.map(e=>`<div class="brief-change"><b>${escHtml(e.ticker)}</b><p>${directionLabel(e.before)} · ${escHtml(e.before.horizon)} ${briefLink(e.before)}<br>${escHtml(e.before.text)}</p><p>${directionLabel(e.after)} · ${escHtml(e.after.horizon)} ${briefLink(e.after)}<br>${escHtml(e.after.text)}</p></div>`).join('')}</details>`:'<p class="brief-change-status">未发现明确多空转向</p><p class="brief-muted">限所选范围内同一标的的方向记录，不代表所有观点都保持不变。</p>';
  const latest=b.latest?`<div class="brief-latest"><div class="brief-latest-date">${escHtml(b.latest.date.slice(0,10))}</div><p>${escHtml(b.latest.text)}</p>${briefLink(b.latest,'查看')}</div>`:'<p class="brief-muted">暂无已解读发言</p>';
  const timeline=b.timeline.length?`<details class="brief-timeline"><summary>展开阶段脉络 · 从早期论点到近期判断</summary><p class="brief-muted">${escHtml(b.change_note)}</p>${b.timeline.map(e=>`<div class="brief-milestone"><span>${briefLink(e)}</span><p>${escHtml(e.text)}</p></div>`).join('')}</details>`:'';
  const mention=b.mentions.length?`<details class="brief-mentions"><summary>仅提及 / 方向未确认 · ${b.mentions.length} 个标的</summary><p class="brief-muted">这些名称出现在帖子中，但尚无足够的逐标的方向依据。</p><div>${b.mentions.map(e=>`<a class="security-view" href="/posts/${encodeURIComponent(e.post_id)}">${escHtml(e.ticker)}<span>${escHtml(e.date.slice(0,10))}</span></a>`).join('')}</div></details>`:'';
  return `<article class="stance author-brief" data-author="${escHtml(key)}"><div class="sh"><div class="av" style="background:${c}">${initials(k.name)}</div><div class="brief-identity"><div class="nm">${escHtml(k.name)}</div><div class="brief-meta">${winLabel(win)} · ${b.post_count} 篇推文 · ${b.interpreted_count} 篇有解读${b.last_post_at?' · 更新 '+escHtml(b.last_post_at.slice(5,10)):''}</div></div><a class="brief-posts" href="/posts/?author=${encodeURIComponent(key)}">全部推文 ↗</a></div>
    <div class="brief-body"><div class="brief-columns"><section class="brief-main"><h3>核心观点</h3>${paragraphs}${extra}</section><aside class="brief-aside" aria-label="标的方向"><h3>具体标的怎么看</h3><div class="brief-aside-groups">${securities}</div>${mention}<p class="brief-muted">所选范围内最后一次明确表态。日期为发帖日，点击标的查看依据。</p></aside></div>
    <div class="brief-updates"><section><h3>最近一次说到</h3>${latest}</section><section><h3>观点的变化与延续</h3>${changes}${timeline}</section></div>
    </div></article>`;
}
function renderStance(){
  const period=(AUTHOR_BRIEFS.periods||{})[String(win)];
  const g=document.getElementById('stancegrid');
  if(!period){g.innerHTML='<p>该范围的阶段观点待生成，请查看推文。</p>';return;}
  document.getElementById('consensus').innerHTML=`<p><strong>${winLabel(win)} · 作者观点回顾</strong> <span>${escHtml(period.start.slice(0,10))} — ${escHtml(period.end.slice(0,10))}</span></p><p>按已存逐帖解读整理主线、理由和前后表态；每段可核对原帖。历史覆盖尚未核实，未发言不等于观点不变。</p>`;
  g.innerHTML=ORDER.filter(key=>stanceAuthor==='all'||key===stanceAuthor).map(key=>renderAuthorBrief(key,period.authors[key])).join('');
}
function initStanceAuthor(){
  document.getElementById('stance-author').innerHTML='<option value="all">全部人物</option>'+ORDER.map(key=>`<option value="${key}">${escHtml(KOLS[key].name)}</option>`).join('');
  document.getElementById('stance-author').onchange=e=>{stanceAuthor=e.target.value;renderStance();};
}

function winLabel(w){return ({0:'近 24 小时',0.25:'近 1 周',1:'近 1 个月',3:'近 3 个月',6:'近 6 个月',12:'近 1 年'})[w]}
